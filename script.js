...
function chime() {
  ...
  setTimeout(() => beep(1175, 0.12, 'sine', 0.04), 160);
}
...
function drawShaft(s) {
  ...
  const base = makeSVG('circle', { cx: s.x, cy: s.y, r: s.r + 16, class: 'peg ring' });
  const stud = makeSVG('circle', { cx: s.x, cy: s.y, r: 16, class: 'peg' });
  group.append(base, stud);
  svg.append(group);
}
drawShaft(inputShaft);
drawShaft(outputShaft);
...
function createGear({ id, x, y, R, fixed = false, draggable = false }) {
  ...
  const rim = makeSVG('circle', { class: 'rim', cx: 0, cy: 0, r: R - 6 });
  const teeth = makeSVG('circle', { class: 'teeth', cx: 0, cy: 0, r: R - 1, fill: 'none' });
  const hub = makeSVG('circle', { class: 'hub', cx: 0, cy: 0, r: 12 });

  for (let i = 0; i < 4; i++) {
    const ang = (i * Math.PI / 2);
    const x1 = Math.cos(ang) * 12, y1 = Math.sin(ang) * 12;
    const x2 = Math.cos(ang) * (R - 18), y2 = Math.sin(ang) * (R - 18);
    const spoke = makeSVG('line', { x1, y1, x2, y2, class: 'spoke' });
    g.append(spoke);
  }
  g.append(rim, teeth, hub);
  const mark = makeSVG('circle', { cx: 0, cy: -(R - 10), r: 4, fill: '#ffd089', opacity: .8 });
  g.append(mark);
...
const marks = makeSVG('g', { class: 'marks' });
const markR = gearOutput.R + 28;
for (let i = 0; i < 3; i++) {
  const a = i * 120 * Math.PI / 180;
  const x = outputShaft.x + Math.cos(a) * markR;
  const y = outputShaft.y + Math.sin(a) * markR;
  marks.append(makeSVG('circle', { cx: x, cy: y, r: 6, fill: '#ffd089' }));
}
svg.append(marks);
...
svg.addEventListener('pointerdown', e => {
  const gEl = e.target.closest('.gear.draggable');
  if (!gEl) return;
  const id = gEl.getAttribute('data-id');
  const obj = gears.find(x => x.id === id);
...
svg.addEventListener('pointerup', () => {
  ...
  for (const p of pegs) {
    ...
    if (d < 50) {
      obj.x = p.x; obj.y = p.y; obj.placedOn = p.id; snapped = true;
      p.ring.classList.add('pegGlow');
      beep(600);
      break;
    }
  }
...
function updateTransform(obj) {
  obj.el.setAttribute('transform', `translate(${obj.x},${obj.y}) rotate(${obj.angle})`);
}

// --- Rotación y transmisión
function rotateGear(gear, deltaDeg) {
  gear.angle = (gear.angle + deltaDeg) % 360;
  updateTransform(gear);
}
function transmitRotation() {
  ...
  for (let i = 0; i < allG.length; i++) {
    for (let j = i + 1; j < allG.length; j++) {
      const A = allG[i], B = allG[j];
      const d = dist(A, B);
      if (Math.abs(d - (A.R + B.R)) <= tol) {
        pairs.push([A, B]);
      }
    }
  }
  const visited = new Set();
  const queue = [{ g: gearInput, w: 1 }];
  const derived = new Map();
  derived.set(gearInput, 0);
  while (queue.length) {
    const { g, w } = queue.shift();
    visited.add(g);
    for (const [A, B] of pairs) {
      let other = null, curr = null;
      if (A === g) { curr = A; other = B; }
      else if (B === g) { curr = B; other = A; }
      else continue;
      if (visited.has(other)) continue;
      const ratio = -(curr.R / other.R);
      queue.push({ g: other, w: w * ratio });
      const inAngle = gearInput.angle;
      const ang = (inAngle * w * ratio) % 360;
      derived.set(other, ang);
    }
  }
  for (const g of gears) {
    if (g === gearInput) continue;
    if (derived.has(g)) {
      g.angle = derived.get(g);
      updateTransform(g);
    }
  }
  return pairs;
...
// --- Fase 1: validación tren
function checkPhase1() {
  const pairs = transmitRotation();
  const connected = (a, b) => pairs.some(([A, B]) => (A === a && B === b) || (A === b && B === a));
  const ok =
    ((gearL.placedOn && gearS.placedOn) &&
      connected(gearInput, gearL) &&
      connected(gearL, gearS) &&
      connected(gearS, gearOutput))
    ||
    ((gearL.placedOn && gearS.placedOn) &&
      connected(gearInput, gearS) &&
      connected(gearS, gearL) &&
      connected(gearL, gearOutput));

  if (ok) {
    $('#statusText').textContent = '¡Tren correcto!';
...
svg.addEventListener('pointerdown', e => {
  ...
  if (dist(p, inputShaft) < gearInput.R + 30) {
    spinDrag = { startAngle: Math.atan2(p.y - inputShaft.y, p.x - inputShaft.x), base: gearInput.angle };
  }
});
svg.addEventListener('pointermove', e => {
  if (!spinDrag) return;
...
function checkPhase2() {
  ...
  if (ok) {
    $('#statusText').textContent = '¡Cerradura abierta!';
    $('#statusText').className = 'status ok';
    gearOutput.el.classList.add('targetGlow');
    chime();
